...
=======
    <section id="testimonials" className="relative overflow-hidden py-20">
      ...
      {/* Background */}
      <div aria-hidden className="pointer-events-none absolute right-0 bottom-0 h-[500px] w-[500px] translate-x-1/4 translate-y-1/4 rounded-full bg-[#DCCCAC]/15 blur-[120px]" />
...
=======
          <span className="inline-flex items-center gap-2 rounded-full border border-[#DCCCAC]/50 bg-[#DCCCAC]/20 px-3.5 py-1.5 text-[11px] font-semibold uppercase tracking-widest text-[#546B41]">
>>>>>>> 096e8fb (Initial commit - UI updates)
            Customer stories
          </span>
...
=======
              ...
              viewport={{ once: true, margin: "-50px" }}
              transition={{ delay: i * 0.1, duration: 0.55, ease: "circOut" }}
              whileHover={{ y: -5, boxShadow: "0 20px 40px -12px rgba(84,107,65,0.15)" }}
              className="flex flex-col gap-5 rounded-2xl border border-[var(--app-border)] bg-white/50 p-6 backdrop-blur-sm transition-colors duration-300 hover:border-[#99AD7A]/60"
>>>>>>> 096e8fb (Initial commit - UI updates)
            >
              <StarRow />
...
=======
          {STATS.map((s, i) => (
            <motion.div
              ...
              className="flex flex-col items-center gap-1 bg-white/60 px-6 py-8 text-center backdrop-blur-sm"
            >
              <motion.span
                initial={{ scale: 0.5 }}
                whileInView={{ scale: 1 }}
                ...
                className="text-3xl font-bold text-[var(--app-text)]"
              >
                {s.value}
              </motion.span>
              <span className="text-sm text-[var(--app-text-muted)]">{s.label}</span>
