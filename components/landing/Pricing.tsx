        ...
        <div className="mt-16 grid gap-5 lg:grid-cols-3">
          {PLANS.map((plan, i) => (
            ...
            >
              ...
              {/* CTA — Razorpay checkout for payable plans, Link for others */}
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="mt-8">
                {plan.payable ? (
                  <RazorpayCheckout
                    amountInRupees={plan.priceNum}
                    planName={plan.name}
                    label={plan.cta}
                    onSuccess={(data) => {
                      console.log("Payment verified:", data);
                      // Redirect to onboard or dashboard after successful payment
                    ...
                    className={
                      plan.highlight
                        ? "w-full bg-[#546B41] text-white shadow-md shadow-[#546B41]/20 hover:bg-[#3d5030] hover:shadow-lg hover:shadow-[#546B41]/30"
                        : "w-full border border-[var(--app-border)] bg-white text-[var(--app-text)] hover:bg-[#DCCCAC]/30"
                    }
                  />
                ) : (
                  <Link
